...
package com.junkbyte.console.view
{
	...
	public class MainPanel extends ConsolePanel
	{
		...
		public function addMenu(key:String, f:Function, args:Array, rollover:String):void
		{
			if (key)
			{
				...
				else
				{
					...
					_extraMenus[key] = [f, args, rollover];
				}
		...
		private function addFilterText(txt:String):String
		{
			if (_filterRegExp)
			{
				// need to look into every match to make sure there no half way HTML tags and not inside the HTML tags
				// it self in the match.
		...
		override public function set height(n:Number):void
		{
			...
			_bottomLine.y = n - (fsize + 6);
			//
			_scroll.y = mini ? 6 : fsize + 4;
			var ctrlSize:uint = style.controlSize;
			_scrollHeight = (_bottomLine.y - (ctrlSize * 2)) - _scroll.y;
		...
		private function updateTraceFHeight():void
		{
			...
			_traceField.height = Math.max(0, height - _traceField.y);
		}

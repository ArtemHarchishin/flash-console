...
package com.junkbyte.console
{
	...
	public class ConsoleConfig
	{
		...
		/**
		 *    Password sequence to toggle console's visibility. If password is set, console will start hidden. Set
		 * Cc.visible = ture to unhide at start. Must be ASCII chars. Example passwords: ` OR debug. Password will not
		 * trigger if you have focus on an input TextField.
		 */
		...
		/**
		 * Object linking allows you click on individual objects you have logged to inspect the detials in a specific
		 * view. The down side is that it will take a little more memory to keep a WEAK reference to all objects pass
		 * for logging. Potentially a security risk as users will be able to explore your code interface.
		 */
		 ...
		 * However it will get garbage collected almost straight away which prevents you from clicking on the object
		 * link.
		 ...
		 * Example, if you set 120, you will get 2 mins guaranteed time that any object link will work since it first
		 * appeared. Default is 0, meaning everything is weak linked straight away. Recommend not to use too high
		 * numbers. possibly 120 (2 minutes) is max you should set.
		 *
		 * Example:
		 * <code>
		 * Cc.log("This is a temp object:", new Object());
		 * // if you click this link in run time, it'll most likely say 'no longer exist'.
		 * // However if you set objectHardReferenceTimer to 60, you will get AT LEAST 60 seconds before it become
		 * unavailable.
		 * </code>
		 */
		 ...
		 * <li>Because console's SharedObject is shared across, your filtering settings will carry over to other flash
		 * projects</li>
		 ...
		 * For example, if console is started in root, when a child display is added in root, console will move it self
		 * to the top of root's display list to try to overlay the new child display. - making sure that console don't
		 * get covered.
		 * </p>
		 * <p>
		 * However, if Console's parent display (root in example) is covered by another display (example: adding a
		 * child directly to stage), console will not be able to pull it self above it as it is in root, not stage. If
		 * console is added on stage in the first place, there won't be an issue as described above. Use
		 * Cc.startOnStage(...).
		 * </p>
		 * <p>

	...
	public class KeyBinder extends ConsoleCore
	{
		...
		public function bindKey(key:KeyBind, fun:Function, args:Array = null):void
		{
			...
			else
			{
				...
				//if(!config.quiet) report("Bined key "+key.key+" to a function."+(config.keyBindsEnabled?"":" (will
				// not trigger while key binding is disabled in config)"), -1);
			}
		}
			...
			if (isKeyUp && config.keystrokePassword != null && char && char == config.keystrokePassword.substring(_passInd, _passInd + 1))
			{
				...
				if (_passInd >= config.keystrokePassword.length)
				{
					...
					if (canTrigger())
					{
						...
						{
							Starling.current.stage.touchable = touchable;
						}
					}
					else if (_warns < 3)
					{
		...
		{
			// in try catch block incase the textfield is in another domain and we wont be able to access the type...
			// (i think)
			try
			{
